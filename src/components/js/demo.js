...
Demo = {
  init: function () {
    ...
    Demo.el.blocks.keypress(function (ev) {
      if (ev.which == 13) Demo.run(); // run on <enter> while entering block information
    });
  },
  ...
  report: function (blocks, w, h) {
    ...
    Demo.el.nofit
      .html("Did not fit (" + nofit.length + ") :<br>" + nofit.join(", "))
      .toggle(nofit.length > 0);
  },
  ...
  sort: {
    ...
    now: function (blocks) {
      ...
      if (sort != "none") blocks.sort(Demo.sort[sort]);
    },
  },
  ...
  blocks: {
    examples: {
      ...
      //Cars up to 1.90 m in height and up to 4 m in length and 1.6-1.7 meters in width (this is typical dimension of a car and we will use it as our sample size. Number of cars can be between 20-40)
      tall: [
        { w: 160, h: 410, num: 3 },
        { w: 165, h: 380, num: 2 },
        { w: 162, h: 400, num: 1 },
      ...
      tallwide: [
        /* alternate tall then wide */ { w: 400, h: 100 },
        { w: 100, h: 400 },
        { w: 400, h: 100 },
        { w: 100, h: 400 },
        { w: 400, h: 100 },
      ...
      change: function () {
        Demo.el.blocks.val(
          Demo.blocks.serialize(Demo.blocks.examples.current())
        );
        Demo.run();
      },
    },
